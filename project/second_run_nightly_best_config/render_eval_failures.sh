...
# Replay failures from an existing final-model evaluation CSV.
#
# Usage:
#   CUDA_VISIBLE_DEVICES=0 ./project/second_run_nightly_best_config/render_eval_failures.sh 0 carla
#   CUDA_VISIBLE_DEVICES=1 ./project/second_run_nightly_best_config/render_eval_failures.sh 1 nuplan_single offroad
#   CUDA_VISIBLE_DEVICES=2 ./project/second_run_nightly_best_config/render_eval_failures.sh 2 nuplan_multi collision 10
#
# Failure modes: all_infractions (default), collision, at_fault_collision,
#                offroad, red_light
...
case "$2" in
    carla|nuplan_single|nuplan_multi)
        DATASET="$2"
        ;;
    *)
...
case "${FAILURE_MODE}" in
    ...
    collision)
        ...
        ;;
    at_fault_collision)
        RENDER_FILTER="at_fault_collision_rate"
        ;;
    offroad)
        RENDER_FILTER="offroad_rate"
        ;;
    red_light)
        RENDER_FILTER="red_light_violation_rate"
        ;;
    *)
...
EXPERIMENT_ROOT="${REPO_ROOT}/experiments/second_run_nightly_best_config"
BENCHMARK_CONFIG="${SCRIPT_DIR}/evaluation_benchmarks.yaml"

shopt -s nullglob
RUN_DIRS=("${EXPERIMENT_ROOT}"/*_seed"${SEED}")
shopt -u nullglob

if (( ${#RUN_DIRS[@]} != 1 )); then
    echo "Expected exactly one experiment directory ending in _seed${SEED}; found ${#RUN_DIRS[@]}." >&2
    exit 1
fi
...
if (( ${#METRICS_FILES[@]} == 0 )); then
    echo "No final-model metrics CSV found for seed ${SEED}, dataset ${DATASET}." >&2
    exit 1
fi

# Timestamped evaluation directories sort chronologically, so use the newest CSV.
...
ANALYSIS_ROOT="${METRICS_DIR}/failure_analysis/${FAILURE_MODE}"
ANALYSIS_DIR="${ANALYSIS_ROOT}/${RUN_TIMESTAMP}"
ANALYSIS_CSV="${ANALYSIS_DIR}/episode_metrics.csv"
INDEX_PATH="${ANALYSIS_DIR}/failures/rendered_replays/index.html"

# The evaluator writes failure artifacts beside failure_replay_csv. Give each
# invocation its own directory so different filters never mix stale replays.
if [[ "${DRY_RUN}" == "0" ]]; then
