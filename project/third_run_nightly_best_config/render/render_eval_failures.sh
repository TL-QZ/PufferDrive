...
# Render selected failures from an existing third-run CARLA-checkpoint evaluation.
#
# Usage:
#   CUDA_VISIBLE_DEVICES=0 ./project/third_run_nightly_best_config/render/render_eval_failures.sh 0 carla
#   CUDA_VISIBLE_DEVICES=1 ./project/third_run_nightly_best_config/render/render_eval_failures.sh 1 nuplan_single offroad
#   CUDA_VISIBLE_DEVICES=2 ./project/third_run_nightly_best_config/render/render_eval_failures.sh 2 nuplan_multi collision 10
...
shopt -s nullglob
RUN_DIRS=("${EXPERIMENT_ROOT}"/nightly_best_local_2gpu_*_seed"${SEED}")
shopt -u nullglob
if (( ${#RUN_DIRS[@]} != 1 )); then
    echo "Expected exactly one third-run CARLA run for seed ${SEED}; found ${#RUN_DIRS[@]}." >&2
    exit 1
fi
