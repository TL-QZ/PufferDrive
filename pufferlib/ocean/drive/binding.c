...
static PyObject *my_shared(PyObject *self, PyObject *args, PyObject *kwargs) {
    ...
    Drive timing_config = {0};
    timing_config.resample_replay_to_dt = PyDict_GetItemString(kwargs, "resample_replay_to_dt")
        ? (bool) unpack(kwargs, "resample_replay_to_dt") : false;
    if (timing_config.resample_replay_to_dt) {
        timing_config.dt = (float) unpack(kwargs, "dt");
        timing_config.scenario_length = (int) unpack(kwargs, "scenario_length");
        timing_config.init_step = init_step;
        timing_config.init_step_spread = PyDict_GetItemString(kwargs, "init_step_spread")
            ? (bool) unpack(kwargs, "init_step_spread") : false;
        timing_config.simulation_mode = simulation_mode;
        if (validate_replay_timing_config(&timing_config) != 0) {
            PyErr_SetString(PyExc_ValueError, timing_config.load_error);
            return NULL;
        }
    }
    uint64_t seed;
    ...
    while (total_agent_count < num_agents && env_count < max_envs
        ...
        env->resample_replay_to_dt = timing_config.resample_replay_to_dt;
        env->dt = timing_config.dt;
        env->scenario_length = timing_config.scenario_length;
        env->init_step_spread = timing_config.init_step_spread;
        if (load_map_binary(map_file, env) != 0) {
            PyErr_Format(PyExc_ValueError, "Scenario %s: %s", map_file, env->load_error);
            free(env);
...
static int my_init(Env *env, PyObject *args, PyObject *kwargs) {
    ...
    env->resample_replay_to_dt = PyDict_GetItemString(kwargs, "resample_replay_to_dt")
        ? (bool) unpack(kwargs, "resample_replay_to_dt") : false;
    env->init_step_spread = PyDict_GetItemString(kwargs, "init_step_spread")
        ? (bool) unpack(kwargs, "init_step_spread") : false;
    ...
    if (init(env) != 0) {
        PyErr_Format(PyExc_ValueError, "Scenario %s: %s", env->map_name, env->load_error);
        c_close(env);
        free(env);
        return -1;
    }
