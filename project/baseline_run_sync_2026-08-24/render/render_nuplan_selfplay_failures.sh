...
# Render selected failures from an existing synced-baseline self-play evaluation.
#
# Usage:
#   CUDA_VISIBLE_DEVICES=0 ./project/baseline_run_sync_2026-08-24/render/render_nuplan_selfplay_failures.sh 0 carla
#   CUDA_VISIBLE_DEVICES=1 ./project/baseline_run_sync_2026-08-24/render/render_nuplan_selfplay_failures.sh 1 nuplan_single offroad
#   CUDA_VISIBLE_DEVICES=2 ./project/baseline_run_sync_2026-08-24/render/render_nuplan_selfplay_failures.sh 2 nuplan_multi collision 10
...
shopt -u nullglob
if (( ${#RUN_DIRS[@]} != 1 )); then
    echo "Expected exactly one completed synced-baseline self-play run for seed ${SEED}; found ${#RUN_DIRS[@]}." >&2
    exit 1
fi
...
if (( ${#METRICS_FILES[@]} == 0 )); then
    ...
    echo "Run eval/evaluate_nuplan_selfplay.sh for this seed first." >&2
    exit 1
fi
...
EVAL_COMMAND=(
    ...
    "eval.num_agents=300"
    # Mean actions from the discrete policy require a continuous environment.
