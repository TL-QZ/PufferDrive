...
# Render selected failures from a completed standalone fine-tuning evaluation.
# Run evaluate_nuplan_sdc_finetune.sh first to create the source metrics CSVs.
#
# Usage:
#   CUDA_VISIBLE_DEVICES=0 ./project/second_run_nightly_best_config/render_nuplan_sdc_finetune_failures.sh 0 carla
#   CUDA_VISIBLE_DEVICES=1 ./project/second_run_nightly_best_config/render_nuplan_sdc_finetune_failures.sh 1 nuplan_single offroad
#   CUDA_VISIBLE_DEVICES=2 ./project/second_run_nightly_best_config/render_nuplan_sdc_finetune_failures.sh 2 nuplan_multi collision 10
#
...
if (( ${#METRICS_FILES[@]} == 0 )); then
    ...
    echo "Run evaluate_nuplan_sdc_finetune.sh for this seed first." >&2
    exit 1
fi

# Timestamped evaluation directories sort chronologically; use the newest CSV.
...
# The evaluator writes failure artifacts beside failure_replay_csv. Isolate each
# invocation so different filters never mix stale replay files.
if [[ "${DRY_RUN}" == "0" ]]; then
